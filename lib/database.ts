...
export class Database {
    ...
    public async query<T = any>(sql: string, databaseId: string, apiKey: string, params?: Record<string, any>, skipCache: boolean = false): Promise<ApiResponse<T>> {
        ...
        try {
            ...
            return {
                ...
                error: '',
                affectedRows: response?.rowsAffected?.[0] ?? 0,
                queryTime: Date.now() - startTime,
                totalRows: response?.recordset?.length ?? 0,
            } as ApiResponse<T>;
        } catch (error) {
    ...
    private async executeQuery<T = any>(sql: string, databaseId: string, apiKey: string, params?: Record<string, any>): Promise<IResult<T>> {
        try {
            ...
            console.error('Query error:', error);
            throw error; // Let the parent function handle the error formatting
        }
    }
}