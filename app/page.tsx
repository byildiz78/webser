...
export default function Home() {
  ...
  const [databases, setDatabases] = useState<SQLConfig[] | undefined>([]);
        ...
        {databasesLoading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto mb-16">
              {selectedDatabase && !healthLoading && health?.serverConnection && health?.databaseConnection && (
                <>
              ...
              <button onClick={handleLogout} className="group col-span-full">
                <div className="bg-gray-800/30 backdrop-blur-lg p-6 rounded-xl border border-gray-700/50 shadow-lg group-hover:shadow-xl group-hover:border-red-500/50 transition-all duration-300">
                  <div className="flex items-center space-x-4">
                    <div className="p-3 bg-red-500/10 rounded-lg group-hover:bg-red-500/20 transition duration-300">
                      <BiLogOut className="w-6 h-6 text-red-500" />
                    </div>
                    <div>
                      <h3 className="text-lg font-semibold text-left">Çıkış Yap</h3>
                      <p className="text-gray-300">Oturumu sonlandır</p>
                    </div>
                  </div>
                </div>
              </button>
